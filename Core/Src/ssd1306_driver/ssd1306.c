/* ssd1306.c — rotated 90° clockwise version
 *
 * This file is adapted from the original stm32-ssd1306 implementation.
 * Rotation is implemented in software by mapping logical (x,y) to
 * physical buffer (px,py) before setting/clearing bits in SSD1306_Buffer.
 *
 * Rotation: 90° clockwise:
 *   logical_width  = SSD1306_HEIGHT
 *   logical_height = SSD1306_WIDTH
 *
 * Physical buffer / hardware settings left unchanged.
 */
...
/* --- NOTE:
 * This implementation rotates the logical coordinate system 90° clockwise.
 * That means:
 *   - Call ssd1306_DrawPixel(x,y) with x in [0 .. SSD1306_HEIGHT-1]
 *     and y in [0 .. SSD1306_WIDTH-1]
 *   - Cursor/strings/etc. use the rotated logical coordinate system.
 */
...
/*
 * DrawArc (logical).
 * Angle handling unchanged; uses ssd1306_Line which maps coordinates.
 */
...
/*
 * Draw arc with radius lines (logical)
 */
...
/* Draw filled circle. Pixel positions calculated using Bresenham's algorithm (logical) */
void ssd1306_FillCircle(uint8_t par_x,uint8_t par_y,uint8_t par_r,SSD1306_COLOR par_color) {
    ...
    do {
        for (int32_t _y = (par_y + y); _y >= (par_y - y); _y--) {
            for (int32_t _x = (par_x - x); _x <= (par_x + x); _x++) {
                if (_x >= 0 && _y >= 0 && _x < SSD1306_ROTATED_WIDTH && _y < SSD1306_ROTATED_HEIGHT) {
                    ssd1306_DrawPixel((uint8_t)_x, (uint8_t)_y, par_color);
                }
            }
        }
...
/* Draw a filled rectangle (logical) */
void ssd1306_FillRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, SSD1306_COLOR color) {
    ...
    for (uint16_t y = y_start; (y <= y_end) && (y < SSD1306_ROTATED_HEIGHT); y++) {
        for (uint16_t x = x_start; (x <= x_end) && (x < SSD1306_ROTATED_WIDTH); x++) {
...
/* Invert rectangle (logical) — per-pixel, safe for rotated buffer */
SSD1306_Error_t ssd1306_InvertRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2) {
    if ((x2 >= SSD1306_ROTATED_WIDTH) || (y2 >= SSD1306_ROTATED_HEIGHT)) {
        return SSD1306_ERR;
    }
    if ((x1 > x2) || (y1 > y2)) {
        return SSD1306_ERR;
    ...
    for (uint16_t lx = x1; lx <= x2; ++lx) {
        for (uint16_t ly = y1; ly <= y2; ++ly) {
            uint8_t px, py;
            ssd1306_map_logical_to_physical((uint8_t)lx, (uint8_t)ly, &px, &py);
            uint32_t idx = (uint32_t)px + ((uint32_t)(py / 8) * SSD1306_WIDTH);
            uint8_t mask = (uint8_t)(1 << (py % 8));
...
/* Draw a bitmap (logical coordinates)
 * Bitmap is expected in conventional row-major bits format as before.
 */
void ssd1306_DrawBitmap(uint8_t x, uint8_t y, const unsigned char* bitmap, uint8_t w, uint8_t h, SSD1306_COLOR color) {
    ...
    for (uint8_t j = 0; j < h; j++, y++) {
        for (uint8_t i = 0; i < w; i++) {
            ...
            if (byte & 0x80) {
                if ((x + i) < SSD1306_ROTATED_WIDTH && (y) < SSD1306_ROTATED_HEIGHT) {
